---
name: jv-chromatic-gesture-zine-v1-1
description: "Transform a user-supplied photo into a vertical 3:5 Chromatic Gesture Zine poster inheriting from scenes-gathered-zine-v1-3, where truthful photography is anchored inside an expansive warm beige paper field, surrounding environment is simplified into halftone dots, stipple, flat graphic forms or print silhouettes, and a SINGLE vivid continuous high-chroma accent gesture line (magenta, royal blue, vermilion red, or neon pink) physically crosses the hand-torn fibrous photo edge to bridge photography, paper illustration, and floating microtext. Use when the user wants a quiet, tactile indie-zine paper poster with a hand-torn photo cutout, active negative space, halftone/woodblock graphic simplification, and a striking high-chroma line trajectory."
---

# 高彩轨迹拼贴纸刊 · Chromatic Gesture Zine v1.1

Create a calm, tactile poster from a supplied photo inheriting the full architecture of **scenes-gathered-zine-v1-3** with a signature **High-Chroma Gesture Line** cross-boundary accent. Preserve the core tenets: **真景为锚、插画成场、色彩成结构、手撕成界、高彩跨界、纸面会呼吸**:
...
- add ONE continuous vivid high-chroma accent gesture line (magenta, royal blue, vermilion red, or neon pink) that physically CROSSES the torn paper edge boundary, moving from illustration field into photo region or vice versa;
...
7. Add ONE continuous high-chroma gesture line (magenta, royal blue, vermilion red, or neon pink) that physically crosses the torn paper boundary to link illustration, photo, and micro-text.
...
- **Gesture line trajectory:** line path derived from the scene's dominant gesture direction, specifying exact color (magenta, royal blue, vermilion red, or neon pink), torn-edge crossing point, approximate length, and whether it carries micro-text along its path.
...
Resolve these 13 visible fields in order:
...
10. **High-Chroma Gesture Line:** mandatory single continuous trajectory line in vivid magenta, royal blue, vermilion red, or neon pink; derived from the scene's dominant gesture direction; physically crossing the torn paper edge boundary (moving from illustration field into photo region or vice versa); specifying exact color, trajectory path, edge crossing point, approximate length, and whether micro-text is carried along its path.
11. **Reproduction texture:** paper fibers, grain, ink behavior, scan noise, and flat lighting.
12. **Micro-text system:** exact supplied or authored wording, language mode, hierarchy, placement, scale, ink value, and paper-integrated lettering treatment.
...
## High-Chroma Gesture Line Engine

The **High-Chroma Gesture Line** is the single defining accent mechanism of this skill. It bridges photography, abstract paper illustration, and micro-text across the hand-torn boundary.

### Rules of the Gesture Line

1. **Vivid Color Selection:** Select exactly ONE color from four options: vivid magenta, royal blue, vermilion red, or neon pink. Keep saturation high (uncapped, high-chroma, neon accent quality).
2. **Source Gesture Origin:** The line trajectory MUST be derived from the scene's dominant gesture direction (e.g. following a coastline contour, architectural diagonal, water spray trajectory, horizon slope, or path convergence).
3. **Physical Boundary Crossing:** The line MUST physically cross the hand-torn fibrous photo edge boundary, transitioning from the warm beige paper illustration field into the realistic photographic zone (or vice versa).
4. **Line Character:** Specify as a single continuous line, streak, or fine dashed trajectory mark. Avoid multiple competing lines.
5. **Micro-Text Relation:** Specify whether the micro-text system runs parallel along the gesture line, terminates at its tip, or floats anchored to its origin.
...
- Let illustration, chromatic structure, or the High-Chroma Gesture Line cross, emerge from, or pass behind selected torn segments, while keeping the paper tear itself readable.
...
- Let photography, illustration, chromatic structure, and the High-Chroma Gesture Line share at least one source-derived contour, axis, rhythm, or field.
...
- Alternatively, align it directly along or anchored to the High-Chroma Gesture Line trajectory.
...
3. **Illustration field, chromatic structure, torn edge, high-chroma gesture line, and micro-text:** retain/merge/omit/transform/expose decisions; illustration grammar, complexity-compression target, field extent, and active density; chromatic integration mode, source shape, exact hue, material, opacity, function, and area; visible hand-torn contour, fibrous band, affected perimeter, and subordinate residue; **high-chroma gesture line (exact color [magenta, royal blue, vermilion red, or neon pink], trajectory path derived from scene gesture, torn-edge crossing point, length, micro-text carrying status)**; exact text, language mode, form, separator, hierarchy, lettering, ink value, scale, and placement.
...
9. Build a visible hand-torn fibrous edge at the main photo–paper handoff; set its contour, fiber band, affected perimeter, and subordinate residue.
10. Define the **High-Chroma Gesture Line**: exact color (magenta, royal blue, vermilion red, neon pink), trajectory path derived from scene gesture, torn-edge crossing point, length, and micro-text relationship.
11. Resolve the language mode, text form, exact micro-text, separator system, hierarchy, and quiet-paper placement (or gesture line alignment); use supplied wording verbatim when available, otherwise default to English-only.
12. Compile the four-paragraph final prompt.
13. Generate with the supplied photo as reference.
14. Inspect at normal and thumbnail scale.
15. Regenerate once with a targeted correction if necessary.
16. Return the generated image plus one brief creative rationale; include the prompt or detailed notes only when the user asks.
...
- **Missing or broken gesture line:** ensure one vivid high-chroma line (magenta, royal blue, vermilion red, or neon pink) physically crosses the torn edge boundary.
...
```markdown
![Chromatic Gesture Zine v1.1 poster](absolute-image-path-or-rendered-image)

**创作思路**

[One short Chinese paragraph explaining the source-derived composition, enlarged illustration field, high-chroma gesture line trajectory crossing the torn edge, and structural role of the added hue.]
```
...
- Eye path: [entry → photographic anchor → high-chroma gesture line / illustration passage → quiet exit]
...
- High-Chroma Gesture Line: [exact color / trajectory path / torn edge crossing point / length / micro-text status]
...
- Is there ONE continuous vivid high-chroma gesture line (magenta, royal blue, vermilion red, or neon pink)?
- Does the High-Chroma Gesture Line physically CROSS the hand-torn photo edge boundary?
- Are exact color, trajectory, crossing point, length, and micro-text relationship specified?
